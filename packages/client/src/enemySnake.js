...
export function syncEnemyMeshes(gameState, frameDelta) {
    ...
    Object.keys(enemyMeshes).forEach(id => {
        ...
        if (!enemies.list.find(e => e.id === enemyId)) {
            enemyMeshes[enemyId].forEach(mesh => {
                if (mesh.geometry) mesh.geometry.dispose();
                if (mesh.material) {
                    if (Array.isArray(mesh.material)) {
                        mesh.material.forEach(material => material.dispose());
    ...
    enemies.list.forEach(enemy => {
        ...
        if (!meshes || meshes.length !== enemy.snake.length) {
            ...
            const newMeshes = [];
            enemy.snake.forEach((pos, index) => {
                const mesh = createSnakeSegmentMesh(pos, index === 0, materials, false, enemy.id);
                if (mesh) {
                    newMeshes.push(mesh);
                    scene.add(mesh);
                }
...
export function checkEnemyCollision(position, gameState, isPlayerHead = false) {
    ...
    const coreCollision = checkEnemyCollisionCore(enemies, position);
    if (coreCollision) {
        const enemy = enemies.list.find(e => e.id === coreCollision.id);
        if (!enemy) return { collided: false };

        const j = coreCollision.segmentIndex;
